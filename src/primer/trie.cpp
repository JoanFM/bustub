...
auto Trie::Get(std::string_view key) const -> const T * {
  ...
  for (auto it = key.begin(); it != key.end(); ++it) {
    char k = *it;
    auto child = node->children_.find(k);
...
auto Trie::Put(std::string_view key, T value) const -> Trie {
  ...
  if (!root_) {
    ...
    // Create the leaf node with the value, and then iterate backwards to create new node where the children is that one
    ...
    for (auto it = key.rbegin(); it != key.rend(); ++it) {
      ...
      // I can move because I know new_node will be gone soon.
  ...
  // So here the stack contains all the nodes in the path
